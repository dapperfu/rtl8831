...
/**
 * the function will initializing 8851bu specific data and hw configuration
 */
enum rtw_hal_status hal_trx_init_8851bu(struct hal_info_t *hal)
{
...
/**
 * the function will deinitializing 8851bu specific data and hw configuration
 */
static void hal_trx_deinit_8851bu(struct hal_info_t *hal)
{
}

static u8 hal_mapping_hw_tx_chnl_8851bu(struct hal_info_t *hal,
...
static enum rtw_hal_status hal_query_info_8851bu(struct hal_info_t *hal, u8 info_id, void *value)
{
...
static enum rtw_hal_status hal_pltfm_tx_8851bu(void *hal,
...
u32
hal_get_wd_len_8851bu(struct hal_info_t *hal, struct rtw_xmit_req *tx_req)
{
 ...
 */
enum rtw_hal_status
hal_fill_wd_8851bu(struct hal_info_t *hal,
                   struct rtw_xmit_req *tx_req,
                   u8 *wd_buf,
                   u32 *wd_len)
{
...
enum rtw_hal_status
hal_usb_tx_agg_cfg_8851bu(struct hal_info_t *hal, u8* wd_buf, u8 agg_num)
{
	return hal_mac_usb_tx_agg_cfg(hal, wd_buf, agg_num);
}

enum rtw_hal_status
hal_usb_rx_agg_cfg_8851bu(struct hal_info_t *hal, u8 mode, u8 agg_mode,
	u8 drv_define, u8 timeout, u8 size, u8 pkt_num)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	/*u8 drv_define, u8 timeout, u8 size, u8 pkt_num*/
...
static void _hal_show_tx_failure_rsn_8851bu(u8 txsts)
{
...
u16 hal_handle_rx_report_8851bu(struct hal_info_t *hal, u8 *rp,
	...
	do {
		...
		*ac_queue = qsel_value % RTW_MAX_AC_QUEUE_NUM;
		...
		if (TX_STATUS_TX_DONE != *txsts) {
			...
			_hal_show_tx_failure_rsn_8851bu(*txsts);
...
u8 hal_get_fwcmd_queue_idx_8851bu(void)
{
	return FWCMD_QUEUE_IDX_8851B;
}

static struct hal_trx_ops ops= {
	.init = hal_trx_init_8851bu,
	.deinit = hal_trx_deinit_8851bu,
	.map_hw_tx_chnl = hal_mapping_hw_tx_chnl_8851bu,
	.get_bulkout_id = hal_get_bulkout_id_8851bu,
	.hal_get_wd_len = hal_get_wd_len_8851bu,
	.hal_fill_wd = hal_fill_wd_8851bu,
	.handle_rx_buffer = hal_handle_rx_buffer_8851b,
	.query_hal_info = hal_query_info_8851bu,
	.usb_tx_agg_cfg = hal_usb_tx_agg_cfg_8851bu,
	.usb_rx_agg_cfg = hal_usb_rx_agg_cfg_8851bu,
	.get_fwcmd_queue_idx = hal_get_fwcmd_queue_idx_8851bu,
	.get_max_bulkout_wd_num = hal_get_max_bulkout_wd_num_8851bu,
	.handle_wp_rpt = hal_handle_rx_report_8851bu,
};

u32 hal_hook_trx_ops_8851bu(struct hal_info_t *hal_info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
